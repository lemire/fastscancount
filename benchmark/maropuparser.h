...
#ifndef FASTSCANCOUNT_MAROPUPARSER_H_
#define FASTSCANCOUNT_MAROPUPARSER_H_
...
public:
  MaropuGapReader(const std::string &filename) : mFilename(filename), fd(NULL) {}
  ...
  template <class container> bool loadIntegers(container &buffer) {
    ...
    if (result != buffer.size()) {
      if (!errno) {
        ...
        throw std::runtime_error("The file appears to be truncated/corrupt!");
      }
      std::stringstream err;
...
private:
  ...
  bool ReadQty(uint32_t &qty) {
    qty = 0;
    if (fd == NULL) {
      throw std::runtime_error("You forgot to open the file.");
    }
    errno = 0;
    ...
    if (qty > 1 << 29) {
      std::cout << "warning: reading a very large array (" << qty
                << " integers) : is your input file in the right format?" << std::endl;
    }
    return true;
  }
